...
class _MainThreadScreenState extends ConsumerState<MainThreadScreen> {
  ...
  void initState() {
    ...
    _messageController.addListener(() {
      debugPrint(
        '🎹 MAIN THREAD - Text controller listener triggered - text: "${_messageController.text}"',
      );
      _handleTextChange();
    });
  ...
  void _handleTextChange() {
    debugPrint(
      '🔤 MAIN THREAD - _handleTextChange called at ${DateTime.now()}',
    );
    ...
    debugPrint(
      '🔤 MAIN THREAD - TEXT CHANGE: hasText=$hasText, _isTyping=$_isTyping, text="${_messageController.text}"',
    );
    ...
    if (hasText && !_isTyping) {
      ...
      _isTyping = true;
      debugPrint(
        '▶️ MAIN THREAD - STARTING TYPING for conversation: ${mainThread.conversationId}',
      );
      ...
      _typingHeartbeat?.cancel();
      debugPrint(
        '⏹️ MAIN THREAD - STOPPING TYPING (text cleared) for conversation: ${mainThread.conversationId}',
      );
      _realtimeService.sendTypingStatus(mainThread.conversationId, false);
      return;
    }
    ...
    if (hasText) {
      _typingTimer = Timer(const Duration(seconds: 15), () {
        if (_isTyping && mainThread != null) {
          ...
          _typingHeartbeat?.cancel();
          debugPrint(
            '⏱️ MAIN THREAD - STOPPING TYPING (fallback timeout) for conversation: ${mainThread.conversationId}',
          );
          _realtimeService.sendTypingStatus(mainThread.conversationId, false);
        }
      });
  ...
  void _startTypingHeartbeat(String conversationId) {
    ...
    _typingHeartbeat = Timer.periodic(const Duration(seconds: 5), (timer) {
      if (_isTyping && _messageController.text.trim().isNotEmpty) {
        debugPrint(
          '💓 MAIN THREAD - TYPING HEARTBEAT for conversation: $conversationId',
        );
  ...
  Future<void> _checkActiveExercise(String conversationId) async {
    try {
      ...
      if (!isMyTurn && prompt.isNotEmpty) {
        ...
        final nameEnd = commaIdx > 0
            ? commaIdx
            : (spaceIdx > 0 ? spaceIdx : prompt.length);
  ...
  Future<void> _sendMessage() async {
    ...
    if (_isTyping) {
      ...
      _typingHeartbeat?.cancel();
      debugPrint(
        '⏹️ MAIN THREAD - STOPPING TYPING (message sent) for conversation: ${mainThread.conversationId}',
      );
      _realtimeService.sendTypingStatus(mainThread.conversationId, false);
    }
  ...
  Widget build(BuildContext context) {
    ...
    if (mainThread == null) {
      return Scaffold(body: Center(child: Text(l10n.failedToLoad)));
    }

    final messagesState = ref.watch(
      messagesProvider(mainThread.conversationId),
    );

    // Auto-scroll when messages are loaded or streaming content arrives
    ref.listen<MessagesState>(messagesProvider(mainThread.conversationId), (
      previous,
      next,
    ) {
          ...
          children: [
            ...
            if (_activeExerciseSession != null)
              ActiveExerciseBanner(
                ...
                currentStep:
                    _activeExerciseSession!['currentStep'] as int? ?? 1,
                totalSteps:
                    (_activeExerciseData?['steps'] as List?)?.length ?? 1,
                  ...
                  context
                      .push(
                        ...
                        extra: {
                          ...
                          'exerciseId':
                              _activeExerciseSession!['exerciseId'] as String,
                        },
                      )
                      .then((_) {
                  ...
                  : ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        if (index < messagesState.messages.length) {
                          ...
                          final mainThread = ref
                              .read(mainThreadProvider)
                              .mainThread;
                ...
                children: [
                  Expanded(
                    child: TextField(
                      ...
                      onChanged: (text) {
                        debugPrint(
                          '🔤 MAIN THREAD - onChanged triggered: "$text"',
                        );
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      children: [
                        ...
                        const SizedBox(height: 16),
                        Text(
                          l10n.pendingInvitationInfo(
                            user!.pendingInvitationEmail!,
                          ),
                          style: Theme.of(context).textTheme.bodyMedium,
                          textAlign: TextAlign.center,
                        ),
